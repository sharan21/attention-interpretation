...
class OrthogonalLSTMCell_hard(RNNCell):
  """Basic LSTM recurrent network cell.
  The implementation is based on: https://arxiv.org/abs/1704.08300
  The implementation is similar to that of basic LSTM cell with some
  subtle differences.
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):
      ...
      c = (c * sigmoid(f + self._forget_bias) + sigmoid(i) *
               self._activation(j))

      temp_h = self._activation(c)*sigmoid(o)

      for hid in self.previous_outputs:
        temp_doc = math_ops.div(math_ops.reduce_sum(math_ops.mul(temp_h, hid),1),math_ops.reduce_sum(math_ops.mul(hid,hid),1) + eps)
        hid_temp = array_ops.transpose(hid)
        t1 = math_ops.mul(hid_temp, temp_doc)
        t1 = array_ops.transpose(t1) 
        temp_h = temp_h - t1

      self.previous_outputs.append(temp_h)
      if self._state_is_tuple:
        new_state = LSTMStateTuple(c, temp_h)
      else:
        new_state = array_ops.concat(1, [c, temp_h])
      return temp_h, new_state
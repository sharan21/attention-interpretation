...
class lstm():
	...
	def __init__(self, l = 64, m = False, f = 128):
		print("creating object")

		self.embed_size = 300
		self.batch_size = 1  # default was 250 for training
		...
		self.checkpoint_to_restore = "/Users/sharan/Desktop/RNN_with_embd/64,False,128.ckpt"
		self.checkpoint_to_create = "/Users/sharan/Desktop/RNN_with_embed/64,False,128.ckpt"

		self.imdb_train_path = "./imdb/train.tsv"
		self.imdb_test_path = "./imdb/test.tsv"

		self.vocab_size = 99426
	...
	def get_gradients(model, predicted_y, test_data,
					  dropout=0.5):  # ensure batch_size of loaded model == number of test cases

		optimizer_here = model.gradients
		embedding_here = model.embedding
		cost_here = model.cost

		gradients, variables = zip(*optimizer_here.compute_gradients(cost_here, embedding_here, gate_gradients=True,
	...
	def build_rnn(n_words, embed_size, batch_size, lstm_size, num_layers,
		...
		with tf.name_scope('predictions'):
			predictions = tf.contrib.layers.fully_connected(dense, num_outputs=1, activation_fn=tf.sigmoid,
															weights_initializer=weights, biases_initializer=biases)
		...
		if (with_embd):
			...
			export_nodes = ['inputs', 'labels', 'embedding', 'embed', 'keep_prob', 'initial_state', 'final_state',
							'accuracy',
				...
				with tqdm(total=len(x_train)) as pbar:
					for _, (x, y) in enumerate(get_batches(x_train, y_train, batch_size), 1):
						...
						summary, loss, acc, state, _ = sess.run(
							[model.merged, model.cost, model.accuracy, model.final_state, model.optimizer], feed_dict=feed)
	...
	def load_and_make_predictions_withargs(lstm_size, multiple_fc, fc_units, vocab_size, embed_size, batch_size, num_layers,
										   dropout, learning_rate, checkpoint, test_data):
		'''Predict the sentiment of the testing data'''
	...
	def train_and_checkpoint(checkpoint_to_create, l, m, f, vocab_size):
		...
		model = build_rnn(n_words=vocab_size, embed_size=embed_size, batch_size=batch_size, lstm_size=l,
						  num_layers=num_layers,
...
if __name__ == '__main__':
	...
	model, all_preds = load_and_make_predictions_single(lstm_size, multiple_fc, fc_units, vocab_size,
